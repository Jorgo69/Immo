/**
 * Get All Users Query — avec pagination, tri avancé et filtrage multi-rôles.
 */
import { ApiProperty } from '@nestjs/swagger';
import { Type } from 'class-transformer';
import { IsOptional, IsInt, Min, Max, IsEnum, IsString, MaxLength, IsBoolean, IsIn } from 'class-validator';
...
export class GetAllQuery {
  ...
  @ApiProperty({ required: false, description: 'Filtrer par plusieurs rôles (séparés par virgule, ex: tenant,landlord)' })
  ...
  @ApiProperty({ required: false, description: 'Champ de tri', enum: ['created_at', 'completion_score'] })
  @IsOptional() @IsIn(['created_at', 'completion_score']) sortBy?: 'created_at' | 'completion_score';
  @ApiProperty({ required: false, description: 'Ordre de tri', enum: ['ASC', 'DESC'] })
